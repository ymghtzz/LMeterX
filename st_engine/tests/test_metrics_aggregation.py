...
class MetricsAggregationTest(unittest.TestCase):
    ...
    def _run_locust_test(self, users: int, duration: int, workers: int = 0) -> Dict[str, Any]:
        """
        运行Locust测试并返回结果
        ...
        cmd = [
            ...
            "--stream_mode", "False",
            "--chat_type", "0",
            "--user_prompt", "Test prompt for metrics validation",
        ]
    ...
    def _validate_metrics_consistency(self, single_process_result: Dict[str, Any], 
                                    multi_process_result: Dict[str, Any]) -> bool:
        """
        验证单进程和多进程结果的指标一致性
    ...
    def test_single_vs_multi_process_consistency(self):
        ...
        for i, case in enumerate(test_cases):
            ...
            # 验证结果一致性
            is_consistent = self._validate_metrics_consistency(single_result, multi_result)
    ...
    def test_time_measurement_accuracy(self):
        ...
        logger.info(f"时间测量测试通过: start={retrieved_start_time}, execution={execution_time}")
...
class MockMetricsTest(unittest.TestCase):
    ...
    def test_worker_metrics_aggregation(self):
        ...
        # 模拟Worker指标数据
        worker_metrics_list = [
            {"pid": 1001, "reqs_num": 10, "completion_tokens": 1000, "all_tokens": 3000},
            {"pid": 1002, "reqs_num": 15, "completion_tokens": 1500, "all_tokens": 4500},
            {"pid": 1003, "reqs_num": 12, "completion_tokens": 1200, "all_tokens": 3600},
        ]
        ...
        logger.info(f"Worker指标聚合测试通过: reqs={total_reqs}, comp_tokens={total_comp_tokens}, all_tokens={total_all_tokens}")
    ...
    def test_tps_calculation(self):
        ...
        logger.info(f"TPS计算测试通过: req_throughput={req_throughput}, completion_tps={completion_tps}, total_tps={total_tps}")
